...
const spectrumSlopeInput = document.getElementById("spectrumSlope");
...
const waveGradientInput = document.getElementById("waveGradient");
...
function slopeOffsetForIndex(index) {
  const slope = Number(spectrumSlopeInput?.value || defaultVisuals.slope);
  return Math.log2(freqAt(index) / 1000) * slope;
}

function displayDbAt(index, db) {
  const signalWeight = Math.max(0, Math.min(1, (db - minDb) / 54));
  return Math.max(minDb, Math.min(maxDb, db + slopeOffsetForIndex(index) * signalWeight));
}

function styledDbAt(index, db) {
  const amp = Number(waveAmpInput.value);
  const compensated = displayDbAt(index, db);
  return Math.max(minDb, Math.min(maxDb, minDb + (compensated - minDb) * amp));
}
...
function shadedColor(color, amount) {
  const rgb = colorToRgb(color);
  const mix = Math.max(0, Math.min(1, amount));
  return `rgb(${Math.round(rgb.r * mix)} ${Math.round(rgb.g * mix)} ${Math.round(rgb.b * mix)})`;
}
...
function makePath(values, floor = height - 20) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const y = dbToY(styledDbAt(i, values[i]));
...
function drawCurve(values, color, alpha = 1, lineWidth = 2, fillAlpha = 0.08) {
  ...
  const fillPercent = Number(waveFillInput.value) / 100;
  const gradientPercent = Number(waveGradientInput.value) / 100;
  const displayFill = Math.max(fillAlpha, fillPercent);
  const area = makePath(values);
  const gradient = ctx.createLinearGradient(0, 0, 0, height);
  gradient.addColorStop(0, displayColor);
  gradient.addColorStop(Math.max(0.08, 0.76 - gradientPercent * 0.56), displayColor);
  gradient.addColorStop(1, gradientPercent > 0 ? shadedColor(displayColor, 1 - gradientPercent * 0.72) : displayColor);
...
function getTrackPeaks(track, limit = 4) {
  ...
  for (let i = 4; i < bins - 4; i += 2) {
    const current = displayDbAt(i, track.spectrum[i]);
    if (current < -58) continue;
    if (current > displayDbAt(i - 2, track.spectrum[i - 2]) && current > displayDbAt(i + 2, track.spectrum[i + 2])) {
      peaks.push({ index: i, db: current, rawDb: track.spectrum[i] });
    }
  }
...
function drawTrackIslands(track, alpha = 0.72, labelMode = "peak") {
  getTrackPeaks(track, 4).forEach((peak, peakNumber) => {
      ...
      const masterY = dbToY(displayDbAt(peak.index, master.spectrum[peak.index]));
...
function drawTerrainTexture() {
  for (let i = 0; i < bins; i += 3) {
    ...
    const displayDb = displayDbAt(i, master.spectrum[i]);
    const y = dbToY(displayDb);
    const h = height - y - 26;
    const alpha = Math.max(0.02, Math.min(0.14, (displayDb - minDb) / 95));
...
function findConflicts() {
  ...
  for (let start = 0; start < bins; start += 8) {
    ...
    const active = visible
      .map((track) => {
        const avg = track.spectrum.slice(start, end).reduce((sum, v, offset) => sum + displayDbAt(start + offset, v), 0) / (end - start);
        return { track, avg };
      })
...
function drawConflicts(zones) {
  ...
  for (const zone of zones.slice(0, 3)) {
    ...
    const y1 = dbToY(styledDbAt(center, first.track.spectrum[center]));
    const y2 = dbToY(styledDbAt(center, second.track.spectrum[center]));
...
function drawConflictBadge(zone, x, y, center) {
  const first = zone.active[0];
  const second = zone.active[1];
  const firstDb = displayDbAt(center, first.track.spectrum[center]).toFixed(0);
  const secondDb = displayDbAt(center, second.track.spectrum[center]).toFixed(0);
...
function inspectAtIndex(index) {
  ...
  const ranked = tracks
    .map((track) => ({ track, db: displayDbAt(index, track.spectrum[index]), rawDb: track.spectrum[index] }))
    .sort((a, b) => b.db - a.db)
    .slice(0, 4);
  const reference = focusedTrack ? tracks.find((track) => track.id === focusedTrack) : null;
  const referenceEntry = reference ? { track: reference, db: displayDbAt(index, reference.spectrum[index]), rawDb: reference.spectrum[index] } : ranked[0];
  ...
  return {
    ...
    masterDb: displayDbAt(index, master.spectrum[index]),
    ranked,
    referenceEntry,
    comparisonEntry,
    delta,
...
function frame(now) {
  ...
  for (const track of tracks) {
    ...
    drawCurve(
      ...
      isSelected ? Math.min(1, Number(waveFillInput.value) / 100 + 0.05) : Number(waveFillInput.value) / 100,
    );
  }
  ...
  if (selectedId) {
    ...
    if (selected) {
      getTrackPeaks(selected, 3).forEach((peak) => {
        drawTrackLabel(selected, (peak.index / (bins - 1)) * width + 8, dbToY(styledDbAt(peak.index, peak.rawDb)) - 22, `${selected.name} ${peak.db.toFixed(0)}`);
      });
    }
  }
...
function drawCurve(values, color, alpha = 1, lineWidth = 2, fillAlpha = 0.08) {
  ...
  gradient.addColorStop(0, displayColor);
  gradient.addColorStop(Math.max(0.08, 0.76 - gradientPercent * 0.56), displayColor);
  gradient.addColorStop(1, gradientPercent > 0 ? "#080b0d" : displayColor);
  ctx.globalAlpha = displayFill;
  ctx.fillStyle = gradient;
  ctx.fill(area);
  ctx.globalAlpha = 1;
